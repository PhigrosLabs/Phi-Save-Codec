...
pub fn binary_struct(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    for field in fields_named.iter() {
        ...
        for attr in &field.attrs {
            let _ = attr.parse_nested_meta(|meta| {
                ...
                Err(meta.error("expected `size_field = ...`, `size_func = ...`, `align = ...`, or `sub_align = ...`"))
            });
        }
        ...
        parse_stmts.push(quote! {
            ...
            let (val, mut l) = <#field_type as BinaryField>::parse(
                &bits[offset..],
                &field_opts
            )?;
            #align_parse_logic
        ...
        build_stmts.push(quote! {
            ...
            let mut field_bv = self.#field_name.build(&field_opts)?;
            #align_build_logic
            bv.extend(field_bv);
        });
    }
    ...
    let expanded = quote! {
        ...
        impl BinaryField for #struct_name {
            ...
            fn build(&self, outer_opts: &Option<Options>) -> Result<BitVec<u8, Lsb0>, String> {
